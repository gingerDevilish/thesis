...
class PicLabeler:
    ...
    def findSSIM(self):
        (score, diff) = compare_ssim(self.image_copy, self.changes_copy, gaussian_weights=True, full=True)
    ...
    def run(self, image, changes):
        ...
        for index, space in enumerate(self.slots):
            for change in self.cer:
                points = [list(change[0]), [change[1][0], change[0][1]], list(change[1]),
                           [change[0][0], change[1][1]]]  # It works, but it shold be simplyfied
    ...
    def iou(self, fig1, fig2):
        ...
        shape1[polygon(fig1[:, 0],
                       fig1[:, 1])] = 1  # some bugs here and at some time this functions do not fill the polygon at all
    ...
    def preprocess_coords(self, xs, ys):
        ...
        for i in range(4):
            distances.append(
                distance.euclidean(np.float32([xs[i], ys[i]]), np.float32([xs[(i + 1) % 4], ys[(i + 1) % 4]])))
